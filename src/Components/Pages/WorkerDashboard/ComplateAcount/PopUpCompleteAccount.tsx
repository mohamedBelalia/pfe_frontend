...
const PopUpCompleteAccount = ({ onCloseComp ,idWorker}: Props) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const response = await Api.get<Commentaire[]>(`/commentaire?id=${formData.idOuvrier}`);
    ...
    const steps = [
        {
            title: isArabicSelected ? "المعلومات الشخصية" : "Personal Information",
            icon: <IoMdPerson className="h-12 mx-2 text-2xl" />,
            content: (
                <div className="flex flex-col md:px-6 md:mt-4">
                    <label className="text-lg flex mt-4 text-blue-700 font-semibold" htmlFor="prenom">{isArabicSelected ? "الاسم الشخصي" : "Prenom"}</label>
                    <input
                        ...
                        id="prenom"
                    />
                    <label className="text-lg flex mt-4 text-blue-700 font-semibold" htmlFor="nom">{isArabicSelected ? "الاسم العائلي" : "Nom"}</label>
                    <input
                        ...
                        id="nom"
                    />
                    <label className="text-lg flex mt-4 text-blue-700 font-semibold" htmlFor="phone">{isArabicSelected ? "الهاتف" : "Telephone"}</label>
                    <input
        ...
        {
            title: isArabicSelected ? "الوصف والصورة" : "Description et l'image",
            icon: <IoImageOutline className="text-2xl mx-2 h-12" />,
            content: (
                <div className="mx-1 md:px-6">
                    <label className="w-auto" htmlFor="img">
                        <p className="text-blue-700 flex w-auto text-lg font-semibold">{isArabicSelected ? "صورة الملف الشخصي" : "Image de Profile"}</p>
                    ...
                    <p className="text-blue-700 flex mt-6 mb-2 text-xl font-semibold">{isArabicSelected ? "الوصف" : "Description"}</p>
                    <textarea
        ...
        {
            title: isArabicSelected ? "المهن" : "Professions",
            icon: <BsTools className="text-2xl mx-2 h-12" />,
            content: (
                <div className="md:px-6">
                    <p className="text-blue-700 flex mt-6 text-lg font-semibold">{isArabicSelected ? "حدد مهنتك" : "Sélectionnez vos professions"}</p>
                    <ChoiseOccupations id={formData.idOuvrier || ''} />
                </div>
            ),
        },
        {
            title: isArabicSelected ? "المدينة" : "Ville",
            icon: <BiSolidCity className="text-2xl mx-2 h-12" />,
            content: (
                <div className="md:px-6">
                    <p className="text-blue-700 flex mt-6 text-lg font-semibold">{isArabicSelected ? "ادخل مدينتك" : "Entrez votre ville"}</p>
                    <ChoiseCity city={isArabicSelected ? formData.ville_AR || '' : formData.ville_FR || ''} onCityChange={handleCityChange} />
                </div>
            ),
        },
    ];
            ...
            <div className="md:w-2/3 px-2 md:px-2 md:pt-12 h-[85vh] md:h-[100vh] relative w-full bg-slate-100">
                ...
                <div className="absolute py-1 w-full flex justify-around left-0  text-xl text-white bottom-10">
                    ...
                    <button onClick={handleSubmit} className="bg-teal-600 w-2/5 md:w-1/3 py-2 rounded-lg px-10">
                        {isArabicSelected ? "تأكيد" : "Valider"}
                    </button>
                </div>
            </div>
        );
    ...
    return (
        <div className="fixed inset-0 z-50 w-full h-full bg-black/80">
            <div dir={isArabicSelected ? "rtl" : "ltr"} className="md:flex-row  md:bg-white m-auto flex-col z-50 inset-0 bg-white container flex h-[100vh]">
                <div className="md:w-1/3 md:h-[100vh] w-full md:px-4 mt-4">
                    <div className="md:block hidden">
                        <div className="flex md:flex-col text-blue-700 font-bold justify-center items-center mb-10">{isArabicSelected ? "أكمل حسابك" : "Complétez votre compte"}</div>
                        <Progress num={num} />
                    </div>
